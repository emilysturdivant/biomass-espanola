...
# Merge ALOS backscatter tiles -------------------------------------------------
# Downloaded from https://www.eorc.jaxa.jp/ALOS/en/palsar_fnf/data/2019/html/Grid24_fnf.htm
year <- 2019
gdalUtils::mosaic_rasters(list.files(path=file.path(results_dir, 'g0nu_HV/Gamma0_lee'), 
                                     pattern='.tif', full.names = TRUE), 
                          file.path(results_dir, str_c('g0nu_HV/g0nu_', year, '_HV_lee.tif')), 
               projwin=c(-74.48133, 20.09044, -68.32267, 17.47022))

# Merge supplemental ALOS mosaic rasters ------------------------------------------------------------
#' Merge ALOS tiles
#' 
#' @param path Directory path for \code{list.files()}
#' @param pattern regex for \code{list.files()} 
#' @param clip_poly \code{sf} polygon to clip output to
#' @param filename Optional filepath to save output
#' @return Merged and clipped raster
#' @examples
#' isl_poly <- sf::st_read(file.path(masks_dir, "vector/hisp18_maskLand_clean.shp"))
...
isl_poly <- readOGR(dsn = file.path(masks_dir, "vector"), layer='hisp18_maskLand_clean')
...
hti_poly %>% st_write("data/contextual_data/HTI_adm/HTI_adm0_fix.shp", append=FALSE)
...
writeRaster(mask_land, file.path(masks_dir, 'hisp18_maskLand.tif'))
mask_land <- raster(file.path(masks_dir, 'hisp18_maskLand.tif'))

# Mask to Haiti extent
msk_land <- terra::rast(file.path(masks_dir, 'hisp18_maskLand.tif'))
hti_poly <- terra::vect("data/contextual_data/HTI_adm/HTI_adm0_fix.shp")
msk_land <- msk_land %>% 
  crop(hti_poly) %>% 
  mask(hti_poly, filename=file.path(masks_dir, 'hti18_maskLand.tif', overwrite=T))

# Load previously created mosaics
dn_linci <- raster(file.path(results_dir, 'tifs_by_R/hisp18_localincidence.tif'))
dn_mask <- raster(file.path(results_dir, 'tifs_by_R/hisp18_mask.tif'))
dn_date <- raster(file.path(results_dir, 'tifs_by_R/hisp18_date.tif'))
plot(dn_linci)
plot(dn_mask)
plot(dn_date)
...
dn_mask <- read_stars(file.path(results_dir, 'tifs_by_R/hisp18_mask.tif'))
rvals <- dn_mask[[1]]
df <- 
...
saveRDS(df, file.path(results_dir, 'R_out/mask_pcts.rds'))
df <- readRDS(file.path(results_dir, 'R_out/mask_pcts.rds'))

# Replicate ALOS Normal mask
msk_A <- read_stars(file.path(results_dir, 'tifs_by_R/hisp18_mask.tif'))
msk_A[msk_A<254] <- NA
msk_A[!is.na(msk_A)] <- 1
msk_A %>% saveRDS(file.path(results_dir, "R_out/mask_ALOS_raster.rds"))
...
g0_fp <- file.path(results_dir, "g0nu_HV/g0nu_2018_HV.tif")
gdal_translate(file.path(results_dir, "g0nu_HV/g0nu_2018_HV_uncropped.tif"), 
               g0_fp, 
               projwin=st_bbox(msk_AWU)[c(1, 4, 3, 2)])
# Load backscatter and set 0 to NA
g0 <- read_stars(file.path(results_dir, "g0nu_HV/g0nu_2018_HV_crop.tif"))
g0[g0==0] <- NA
...
g0.agg <- aggregate(g0.nofilt, fact=2, fun=mean, na.rm=TRUE,
                    filename=file.path(results_dir, "g0nu_HV/g0nu_2018_agg50m.tif"),
                    overwrite=TRUE)
...
mskinv_p3 %>% saveRDS(file.path(results_dir, "R_out/mask_inv_ALOSoverpt3_stars.rds"))
...
mskinv_p29 %>% saveRDS(file.path(results_dir, "R_out/mask_inv_ALOSoverpt29_stars.rds"))
...
mskinv_p2 %>% saveRDS(file.path(results_dir, "R_out/mask_inv_ALOSoverpt2_stars.rds"))
...
g0lee <- read_stars(file.path(results_dir, 'g0nu_HV/Gamma0_lee/Gamma0_2018_N19W075.tif'))
g0agg <- read_stars(file.path(results_dir, 'g0nu_HV/g0nu_2018_haiti_agg50m.tif'))

mskinv_p3 <- readRDS(file.path(results_dir, "R_out/mask_inv_ALOSoverpt3_stars.rds"))
mskinv_p29 <- readRDS(file.path(results_dir, "R_out/mask_inv_ALOSoverpt29_stars.rds"))
...
r <- raster(file.path(results_dir, "g0nu_HV/g0nu_2018_HV_haitiR.tif")) %>% 
  mask(hti_poly, inverse=FALSE) 
r %>% writeRaster(file.path(results_dir, "g0nu_HV/g0nu_2018_HV_hticlip.tif"), overwrite=T)
g0 <- raster(file.path(results_dir, "g0nu_HV/g0nu_2018_HV_hticlip.tif"))

msk_land <- raster(file.path(masks_dir, "hti18_maskLand_clip2border.tif"))
extent(msk_land)
extent(g0)
...
# Load files
g0 <- read_stars(file.path(results_dir, "g0nu_HV/g0nu_2018_HV.tif"))
g0 <- raster(file.path(results_dir, "g0nu_HV/g0nu_2018_HV.tif"))
...
g0 <- crop(g0, hti_poly)
g0[g0==0] <- NA
writeRaster(g0, file.path(results_dir, "g0nu_HV/g0nu_2018_HV_haitiR.tif"), overwrite=TRUE)
g0 <- raster(file.path(results_dir, "g0nu_HV/g0nu_2018_HV_haitiR.tif"))
...
x$code <- c("land", "sea")
levels(f) <- x
writeRaster(f, file.path(masks_dir, "mask_NAinland.tif"))

# Land mask
land <- msk
land[is.na(land)] <- 1
land[land==2] <- NA
writeRaster(land, file.path(masks_dir, "mask_land18.tif"))
land <- raster(file.path(masks_dir, "mask_land18.tif"))
land_poly <- rasterToPolygons(land) # very slow, much faster with gdal_polygonize in QGIS 
land_poly <- readOGR(dsn=file.path(masks_dir, "vector"), layer='mask_land18_hti')
