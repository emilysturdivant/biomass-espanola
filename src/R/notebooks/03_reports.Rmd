...
### Test statistics
We want to see high coefficient of determination (adjusted R2) and low Bias, RMSE, and MAE. 
...
```{r percentiles}
r <- terra::rast(agb_l0_fp)
quants <- global(r, function(x) quantile(x, probs = seq(0.9, 1, 0.01), na.rm = TRUE))
(p99 <- global(r, function(x) quantile(x, probs = 0.99, na.rm = TRUE)))
(p999 <- global(r, function(x) quantile(x, probs = 0.999, na.rm = TRUE)))
(p9999 <- global(r, function(x) quantile(x, probs = 0.9999, na.rm = TRUE)))
```