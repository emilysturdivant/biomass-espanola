...
# Save
ggsave(file.path(comparison_dir, '07_pct_masked_by_lc.png'),
...
# Save
plot_fp <- file.path(comparison_dir,
...
plots_tc %>% saveRDS(file.path(comparison_dir, 'plot_tc.rds'))

# Plots for given mask
(patches <- plots_tc %>% 
  wrap_plots(nrow = 4, guides = 'collect') &
    theme(axis.title = element_blank()))
map_fp <- file.path(comparison_dir, str_glue("scattdens_{mskvals$code}.png"))
...
plots_total %>% saveRDS(file.path(comparison_dir, 'plots_total.rds'))
...
plots_bgs %>% saveRDS(file.path(comparison_dir, 'plots_bgs.rds'))
...
map_fp <- file.path(comparison_dir, str_glue("scattdens_all.png"))
...
out_dir <- file.path(comparison_dir, 'sidebyside')
dir.create(out_dir)

# Ours, with GlobBiomass, ESA, and Baccini
(all_maps <- maps[c(1,3,2,5)] %>% 
  wrap_plots(nrow = 2, guides = 'collect'))
map_fp <- file.path(out_dir, str_glue("sidebyside_Us_ESA_GlobB_Bacc_{resolution}m.png"))
...
map_fp <- file.path(out_dir, str_glue("all5_equal_{resolution}m.png"))
...
(all_maps <- diffmaps %>% 
  wrap_plots(nrow = 2, guides = 'collect') &
    ggthemes::theme_map())
map_fp <- file.path(out_dir, str_glue("difference_maps.png"))
