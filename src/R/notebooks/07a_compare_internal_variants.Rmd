...
```{r pct-na}
smmry_masks_csv <- file.path(comp_dir1, '07_overview_by_mask.csv')
out_by_mask1 <- read_csv(smmry_masks_csv)
out_by_mask1 %>% filter((mask == 'TC' | mask == 'Total') &
                          map_code == 'internal')

smmry_masks_csv <- file.path(comp_dir2, '07_overview_by_mask.csv')
out_by_mask2 <- read_csv(smmry_masks_csv)
out_by_mask2 %>% filter((mask == 'TC' | mask == 'Total') &
                          map_code == 'internal')
```

```{r pix2pix metrics}
compare_by_lc_csv1 <- file.path(comp_dir1, str_c('07_comparison_by_LC.csv'))
p2p_tbl1 <- read_csv(compare_by_lc_csv1)
p2p_tbl1 %>% 
  select(map_code, mask, R2, RMSD, bias, MBD, MAD, cor.coef, mean_agb_ha, pct_of_lc_100m) %>% 
  filter(mask %in% c('Total'))

compare_by_lc_csv2 <- file.path(comp_dir2, str_c('07_comparison_by_LC.csv'))
p2p_tbl2 <- read_csv(compare_by_lc_csv2)
p2p_tbl2 %>% 
  select(map_code, mask, R2, RMSD, bias, MBD, MAD, cor.coef, mean_agb_ha, pct_of_lc_100m) %>% 
  filter(mask %in% c('Total'))
